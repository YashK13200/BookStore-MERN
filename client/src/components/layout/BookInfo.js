...
function BookInfo(props) {
  ...
  useEffect(() => {
    ...
    if (isSubscribed) {
      axios.get(`http://localhost:5000/books/${props.match.params.id}`).then(res => {
        setOldBook({
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/books/${props.match.params.id}/user`).then(res => {
      setBookOwner({
  ...
  const deleteBook = (bookId) => {
    axios.delete("http://localhost:5000/books/" + bookId).then(res => {
